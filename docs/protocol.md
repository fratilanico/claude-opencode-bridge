...
## Stateless bridge rule

Bridged Claude calls are stateless.

- no Claude resume/session persistence is used for the bridged tool loop
- OpenCode sends the live transcript each turn
- the bridge rebuilds the model prompt from OpenCode messages instead of relying on hidden Claude harness memory
